...
所有的运算符与操作数都用格式分开. 如果某个字段是数值, 就把它入栈;
...
\refv{chap:processing_words} 章所描述的生成过程刚好相反. 比如, 表达式 
\texttt{1 + 2 * 3} 的解析过程是:\footnote{TODO}

为了开发中缀计算器, 我们需要一个表达式解析程序. 只需要多花点精力, 
就可以利用语法规则来构造解析器, 除此之外, 它还可以用来组织程序.
写一个函数, 按照语法规则处理每一个非终结符: 程序使用函数 \texttt{expr}
处理由加号或负号分隔的 \textit{term}, 使用函数 \textit{term} 处理 
由乘号或除号分隔的 \textit{factor}, 最后, 使用函数 \textit{factor} 来
识别数学和被括号包围起来的 \textit{expr}.

在下面的程序里, 每一个输入都被当作一个单独的表达式, 表达式被求值并打印
出来.\marginpar{146}
...
\begin{exercise}
    为中缀计算器添加指数运算, 内建函数和变量. 与逆波兰式计算器相比, 在
    实现上会有什么不同之处?
\end{exercise}
...
如果你要为某个语言处理程序设计前端, 一个比较好的开始是为输入语言写一套
语法规则. 利用 \ref{sec:random_text_generation} 节介绍的表示法, 我们可 
以这样描述 awk 子集的语法规则:
\marginpar{148}
\begin{tabbing}
    \textit{program}\hspace{4em}\=$\to\=\textit{opt-begin pa-stats
    opt-end}\\
    ...
    \>\>\texttt{if (}\ \textit{expr}\ \textit{stat opt-else} |\\
    ...
    |\cdots|\textit{expr}\texttt{>}\textit{expr}|\\
    \>\>\textit{expr}\texttt{+}\textit{expr}|\textit{expr}\texttt{-}
    \textit{expr}|\\
    \>\>\textit{expr}\texttt{*}\textit{expr}|\textit{expr}\texttt{/}
...
构造一个小型语言通常是一个非常多产的编程工作. 如果词法和语法分析
可以通过字段分割与正则表达式来完成, 那么使用 awk 就很方便, 符号
表可以用关联数组来存放, \patact 结构与面向模式的编程语言非常般配.

通常来说, 如果缺乏经验, 那么为新的应用领域设计一门新语言将是一件
非常困难的工作. 不过, 利用 awk 很容易就可以构造出语言原型并加以
...
design'', \textit{IEEE Transactions on Software Engineering}, 1987.
...
``Little Languages'', 登在 \texttt{Programming Pearls} 栏目.
