...
If the extra blanks do not distribute evenly, the surplus ones are spread
alternately from the left and the right on successive lines, to avoid
``rivers'' of white space.\footnote{TODO}
\marginpar{202}
\begin{awkcode}
...
面上的字符时, 我们就在字符前加一个反斜杆 \verb'\'. We'll consider only
\verb'~'; the others are just elaborations in both \texttt{ix.genkey} and
\texttt{ix.format}.\footnote{TODO} 对于 \verb'~', 我们把所有的 \verb'\~'
实例替换成
