...
\begin{enumerate}
    \item 树是高度平衡的: 叶子节点最多只在两个不同的层次上出现, 另外,
        位于最底层 (距离根节点最远的层次) 的叶子尽量靠左排列;
    \item 树是部分有序的: 每个节点的值大于或等于它的孩子节点.
\end{enumerate}
这是带有 10 个元素的堆:
\begin{center}
\begin{tikzpicture}
...
堆具有两个非常重要的特性. 第一个特性是, 如果有 \textit{n} 个节点, 那么所
有的从根节点到叶子节点的路径长度都不会大于 $\log_2 n$. 第二个特性是,
具有最大值的元素总是在根节点 (这个位置称为 ``堆顶'').

如果我们用一个数组 \texttt{A} 来模拟堆, 那么就不需要构造显式的二叉树.
树中的节点按照广度优先遍历的顺序出现在数组中, 也就是说, 根节点在
\texttt{A[1]}, 它的左孩子与右孩子分别在 \texttt{A[2]} 和 \texttt{A[3]}.
总的来说, 如果某个节点在 \texttt{A[}\textit{i}\texttt{]}, 那么它的孩子就
...
\end{awkcode}
我们可以把这个过程可视化: 让元素 \texttt{28} 沿着二叉树的路径, 从根节点
开始, 朝着叶子节点方向, 逐渐向下渗透, 直到到达这样一个的节点: 它的孩子
小于或等于 \texttt{28}:
\begin{center}
\begin{tikzpicture}
    \coordinate (p0) at (0, 0);
...
约束条件可以用一张图来表示, 图中的每个节点都用名字标记, 如果节点 \textit{x}
...
对来表示,
其中每个输入行的 \textit{x} 和 \textit{y} 表示一条从节点 \textit{x} 指向
...
拓扑排序的目标是对图中的节点进行排序, 使得所有前驱都在它们的后继之前出现.
\marginpar{171} 当且仅当图中不含有 \cterm{环} (\term{cycle}, 是由多条边
组成的序列, 当节点沿着这个序列前进时, 最终会回到节点的原来位置) 时,
这样的顺序才存在.
...
从图中移除没有前驱的节点. 如果所有的节点都按照这种方式从图中移除出来,
那么图的拓扑排序结果就是节点按照移除顺序所组成的序列. 在上面的图中, 我们
可以从移除节点 \texttt{a} 及以它为起点的边开始, 然后移除节点 \texttt{c},
再接着是节点 \texttt{f} 或 \texttt{h}, 依此类推.

我们的实现使用了一种先进先出的数据结构 --- \cterm{队列} (\term{queue}) ---
来确定不含有前驱的节点的处理顺序, 以一种 ``广度优先'' 的方式进行.
当所有的节点都被读取完毕后, 一个循环体计算节点的数量, 并把所有的, 没有
前驱的节点插入到队列中. 第二个循环体移除队列的第一个节点, 打印该节点的
名字, 然后递减它的每一个后继节点的前驱节点个数. 如果某一个后继节点的前驱
节点个数变为 0, 那就把这个后继节点插入到队尾. 当队列变成空, 并且所有的节点
都被访问过, 这时候拓扑排序就完成了. 但是, 如果有某些节点从未被插入到队
列中, 那就说明这些节点处在环中, 因此无法对该图进行拓扑排序. 如果图中
不存在环, 那么被打印出来的节点序列就是一个拓扑排序.
...
数组 \texttt{pcnt} 和 \texttt{scnt} 记录每个节点的前驱节点与后继节点个数,
\texttt{slist[}\textit{x}\texttt{,}\textit{i}\texttt{]} 给出了节点
\textit{x} 的第 \textit{i} 个后继节点的名字. 如果某个元素原来不在数组
...
问题. 深度优先搜索是另一种遍历图节点的系统化方法, 即使是在含有环的图中.
...
\end{pattern}
之所以称为 ``深度优先'', 是因为搜索从一个节点开始, 然后访问该节点的某个
未被访问过的后继节点, 再接着访问该后继节点的某个未被访问过的后继节点,
依此类推, 基本原则是尽可能快地向图的纵向深入. 如果某个节点的所有后继
节点都被访问过了, 搜索过程就回退到前驱节点, 然后再按照深度优先的方法,
搜索前驱节点的另一个未被访问过的后继节点.

\marginpar{173}
考虑下面的这张图. 如果遍历从节点 1 开始, 那么深度优先搜索将会陆续访问
节点 1, 2, 3 和 4. 这时候, 如果从另一个未访问过的节点开始, 比如 5, 那么
接下来将会访问节点 5, 6 和 7. 如果遍历从不同的节点开始, 就会得到不同
的节点访问序列.
...
的祖先节点, 比如 \texttt{(3, 1)}, 那么这条边就叫作\cterm{回边} (\term{back
edge}). 因为存在回边等同于存在环, 所以为了判断图中是否含有环, 我们只 
需要搜索回边就够了. 下面的函数判断某个图中是否存在节点 \texttt{node}
可到达的环, 图用后继节点列表来表示, 这个数据结构我们已经在 \texttt{tsort}
中见过.
\begin{awkcode}
...
这个函数使用数组 \texttt{visited} 来判断某个节点是否被访问过. 开始时,
\texttt{visited[}\textit{x}\texttt{]} 等于 0. 在第一次进入节点 \textit{x}
时, \texttt{dfs} 把 \texttt{visited[}\textit{x}\texttt{]} 设置为 1,
最后一次离开节点 \textit{x} 时, 把 \texttt{visited[}\textit{x}\texttt{]}
设置为 2. 在遍历过程中, \texttt{dfs} 利用 \texttt{visited} 来判断节点
\textit{y} 是否是当前节点的祖先 (如果是的话, 说明它之前被访问过),
以及判断 \textit{y} 之前是否被访问过. 如果 \textit{y}
是当前节点的祖先, 那么 \texttt{visited[}\textit{y}\texttt{]} 等于 1;
...
函数 \texttt{dfs} 很容易就可以修改为节点排序例程. 从某节点开始的搜索过
程一旦结束, 如果在此时打印该节点的名字, 那么所打印出来的节点序列刚好就
是逆序的拓扑排序 (前提是图中不含有环). 给定一个 \mbox{前驱}-后续\ 对序列
作为输入数据, 程序 \texttt{rtsort} 输出图的逆序拓扑排序, 它对每一个
没有前驱的节点应用深度优先搜索, 程序中使用的数据结构与 \texttt{tsort}
中的相同.
\marginpar{174}
\begin{awkcode}
...
而对另一些环的探测则是隐式地, 对于后一种情况, 程序将无法打印出全部的节点,
就像这幅图:
...
\begin{exercise}
    \label{exer:rtsort}
    修改 \texttt{rtsort}, 按照通常的顺序打印节点, 也就是先打印前驱节点.
...
大型程序包含的声明与子例程通常分布在多个文件里, 通过一系列复杂的处理
步骤来生成最终的程序. 一篇复杂的文档 (比如本章) 可能由许多图表和程序组成,
这些图表和程序代码存放在不同的文件里, 而且程序会被运行和测试, 最终将通过
...
赖关系, 以及构造它们的命令. 我们假设依赖关系和构造命令存放在一个文件里,
...
用来生成 \textit{name} 的命令. 下面显示的是某个小程序的
...
\textit{x} 依赖于右边的目标 \textit{y}, 那么在图中就有一条边从节点
\textit{x} 指向节点 \textit{y}. 如果某条规则没有目标, 就在图中创建一
个没有后继的节点, 并用规则左边的名字来命名. 上文的 \texttt{makefile}
对应的依赖图是:
\begin{center}
\xymatrix {
...
\begin{enumerate}
    \item 
        ...
        $t_i$ 不是一个名字, 并且文件 $t_i$ 不存在, 程序就会终止更新过程.
    \item 
...
名, \texttt{make} 就把它的时间设置成一个很大的值, 以确保它足够老.
