...
拼接. 拼接并没有显式的运算符, 字符串可以通过这些方式创建: 字符串常量, 变量,
数组元素, 函数返回值, 以及其他与之相连的表达式, 程序
\begin{awkcode}
    { print NR ":" $0 }
\end{awkcode}
...
返回值大于等于 0. ---- 译者}.
\marginpar{43}
(\verb'gsub' 的 \verb'g' 是 ``global'' 的缩写, 意思是替换是全局的). 例如,
程序
\begin{awkcode}
...
不存在, 或显式为空的字段具有字符串值 \verb'""', 它们不是数值, 但是当强制
...
突然存在\footnote{原文为 Like variables, array elements sprint into
