...
出现在了同一行, 并且 \stmt 是一条单独的语句, 那么就必须使用封号来终止
\stmt. \verb'do' 循环执行 \stmt 一次, 然后只要 \expr 为真, 就重复执行
\stmt. \verb'do' 循环与 \verb'while' 和 \verb'for' 循环相比有很大的不同:
...
单独一个封号表示一个空语句. 在下面这个程序里, \verb'for' 的循环体是一个
空语句.
\begin{awkcode}
    BEGIN { FS = "\t" }
          { for (i = 1; i <= NF && $i != ""; i++)
...
突然存在\footnote{原谅为 Like variables, array elements sprint into
existence by being mentioned.}; 数组元素的默认初始值为 0 或空字符串
\verb'""'.
...
\end{awkcode}
第一个动作仅仅是将每个输入行存放到数组 \verb'x' 中, 使用行号作为下标注;
真正的工作在 \END 语句中完成.
...
下面这个程序将 \verb'Asia' 与 \verb'Europe' 的人口数量累加上数组 \verb'pop'
中. \END 动作打印这两个洲的总人口.
\begin{awkcode}
    /Asia/      { pop["Asia"] += $3 }
    /Europe/    { pop["Europe"] += $3 }
...
洲的数目有多少个, 这个程序都可以正常的工作; 文件 \filename{countries}
的输出是
\begin{file}
    Europe 172
    Asia 2173
    ...
    North America 340
\end{file}
最近的这个程序使用 \verb'for' 语句的另一种形式来遍历数组下标:
\begin{pattern}
...
\verb'pop["Africa"]', 但是如果你写成下面这种样子, 就会有创建新元素的副
作用:
\begin{awkcode}
    if (pop["Africa"] != "") ...
\end{awkcode}
\emph{\verb'delete' 语句} (\emph{The \verb'delete' Statement}).
一个数组元素可以通过
\begin{pattern}
...
\emph{\verb'split' 函数} (\emph{The \verb'split' Function}). 函数
...
通常形式就变成了一系列的, 由换行符或封号分开的 \patact 语句与函数定义.
...
接收到的参数是变量的值的一份拷贝, 所以函数操作是变量的拷贝, 而不是变量本
...
它们与程序中其他同名的变量没有任何关联. 但是 \emph{所有的其他变量都是全
...
\begin{summary}{输出语句}
    \begin{enumerate}
        ...
        \item \print \\
            将 \verb'$0' 打印到标准输出
        \item \print \expr\verb',' \expr\verb',' \ldots \\
            打印各个 \expr, \expr 之间由 \verb'OFS' 分开, 由 \verb'ORS' 
            终止
        \item \print \expr\verb','\expr\verb','\ldots \verb'>'
            \textit{filename}
            \\ 输出至文件 \textit{filename}
        \item \print \expr\verb','\expr\verb','\ldots \verb'>>'
            \textit{filename}
            \\ 累加输出到文件 \textit{filename}, 不覆盖之前的内容
        \item \print \expr\verb','\expr\verb','\ldots \verb'|'
            \textit{command} \\
...
\print 语句具有形式:
\begin{pattern}
    \print \expr$_1$\verb',' \expr$_2$\verb','...\verb','\expr$_n$
...
初始情况下, \verb'OFS' 与 \verb'ORS' 分别被设置为一个空格符与一个换行符,
但它们的值可以在任何时候改变. 举例来说, 下面这个程序打印每一行的第 1 与第 
2 个字段, 字段之间用封号分开, 在每一行的第 2 个字段之后输出两个换行符.
\begin{awkcode}
    BEGIN   { OFS = ":"; ORS = "\n\n" }
            { print $1, $2 }
\end{awkcode}
...
括号的两种形式:
\begin{pattern}
    \printf\ \fmt\verb','\expr$_1$\verb','\expr$_2$\verb','\cdots\verb','
    \expr$_n$ \par 
    \printf\verb'('\fmt\verb','\expr$_1$\verb','\expr$_2$\verb','
    \cdots\verb',' \expr$_n$\verb')'
\end{pattern}
...
\begin{tabbing}
\marginpar{57}
    \verb'-' \hspace{4em} \= 表达式在它的域内左对齐 \\
    \textit{width}  \> 为了达到规定的宽度, 必要时填充空格; 前导的 \verb'0'
    表示用零填充 \\
    \verb'.'\textit{prec}   \> 字符串最大宽度, 或十进制数的小数部分的位数
\end{tabbing}
\begin{center}
    \begin{figure}
        ...
        \begin{tabular}{c|l}
            ...
            \verb'g' & 按照 \ver'e' 或 \verb'f' 进行转换, 选项较短的那个,
                无意义的零会被抑制 \\
            \verb'o' & 无符号八进制数 \\
            \verb's' & 字符串 \\
            \verb'x' & 无符号十六进制数 \\
...
\begin{center}
    \begin{figure}
        ...
        \begin{tabular}{c|l|l}
            \hline
            \hline
            \verb'fmt'  & \verb'$1'     & \verb'printf(fmt, $1)' \\
            \hline
...
输送给 \verb'sort' 命令\footnote{正确的写法应该是 \texttt{sort -t'\t' 
-k1rn}}
\begin{awkcode}
...
\expr 指定; \expr 的字符串值必须与用于创建文件或管道的字符串值相同. 于是 
\begin{awkcode}
    close("sort -t'\t' +1rn")
\end{awkcode}
关闭上面打开的排序管道.

在同一个程序中, 如果你写了一个文件, 而待会想要读取它, 那么就需要调用
...
它会设置 \verb'NF', \verb'NR', 和 \verb'FNR'; 如果存在一个记录, 返回 1,
若遇到文件末尾, 返回 0, 发生错误时返回 -1 (例如打开文件失败).
...
\verb'who' 命令的输出是所以已登录用户的用户名. \verb'while' 循环的每一次
迭代, 从这个用户名列表里读取一行或多, 并递增变量 \verb'n', 当循环结束后,
...
\end{awkcode}
这个循环只有在 \verb'getline' 返回 1 的情况下才会执行.
...
\begin{pattern}
    ...
    \indent\verb"awk -F"\textit{sep}\verb" '"\textit{program}
        \verb"' f1 f2 ..." \par
...
执行赋值动作. 这种类型的赋值语句允许变量在文件读之前或之后改变.
...
\verb'b'. \verb'ARGC' 之所以会比参数的个数多1, 是因为命名的名字 \verb'awk'
也被当作参数之一, 存放在索引为 0 的位置, 就像 C 程序那样. 然而, 如果 awk 
程序出现在的命令行, 那它就不会被当作参数, 对 \verb'-f '\textit{filename}
...
空, 意味着 awk 不会把它看作是输入文件. 名字 ``\verb"-"'' 可以当用标准输入
来用.
...
\textit{expression} 给出. \verb'system' 的返回就是命令的退出状态.
...
把 awk 程序包围在一对单引号中可以确保 Shell 把程序原封不动的传递给 awk.

上面提到的两种执行 awk 程序的方法都要求用户自己打一些字. 为了降低打字的
工作量, 我们想要把命令与程序都写到一个可执行文件中, 通过键入文件名来执行
程序. 假设我们想要写一个命令 \verb'field1', 用于打印每个输入行的每一个
字段, 其实这非常容易: 我们把
\begin{awkcode}
    awk '{ print $1 }' $*
\end{awkcode}
...
现在我们考虑实现一个更加通用的 \verb'field', 程序可以从每个输入行
打印字段的任意组合; 换句话说, 命令 
\begin{pattern}
    \indent\verb'field '$n_1$\ $n_2$\ ...\ \textit{file}$_1$\ \textit{file}
        $_2$\ ...
\end{pattern}
按照特定的顺序打印特定的字段. 但是我们如何获取 $n_i$ 的值, 又如何区分
$n_i$ 与文件名参数?

对于采用 Shell 编程的人来说, 解决上面提到的两个问题有多种方式. 如何使用 
awk 的话, 最简单的方式是扫描内建数组 \verb'ARGV', 获取 $n_i$ 的值之后, 把
数组中对应的位置清零, 这样它们就不会被当成文件名.
\marginpar{66}
\begin{awkcode}
...
再看一下某些小节, 有时候是为了精确的了解程序如何工作, 有时候则是因为后面
的章节出现了一些你之前没有试过的构造方法.

和其他语言一样, awk 也需要通过经验与实践来学习, 所以我们鼓励你开始自己写
程序. 程序不需要有多么的庞大或复杂 ---- 你可以通过仅仅几行代码, 来学习
语言的某些特性如何工作, 或测试程序的某些关键点, 你也可能自己手动输入数据
来查看程序的运行效果.
...
Prentice-Hall 1978 年出版) 对 C 程序设计语言进行了完整的描述. 关于如何 
